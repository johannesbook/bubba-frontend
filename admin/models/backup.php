...
class Backup extends Model {
	...
	function get_restorestatus() {
		$lockfile = "/var/lock/restore.lock";
		$status = array(
			'jobname' => '',
			'user' => '',
			'restoring' => false,
		...
		if( file_exists( $lockfile ) && filesize($lockfile) > 0 ) {
			...
			list( $user, $job, $logfile ) = explode(' ', file_get_contents( $lockfile ) );
			$status['user'] = $user;
			$status['jobname'] = $job;
			...
			if( file_exists( $logfile ) && filesize( $logfile ) > 0 ) {
				...
				if( is_resource( $handle ) ) {
					...
					$state_comment = 2;
					$state = $state_comment;
					$type = $level = $rest = $comment = '';

					while( ($line = @fgets($handle)) !== false ) {

						if(preg_match("#^(\.|\s*$)#mS", $line)) {
							...
							if( $state == $state_keyword ) {
								...
								case 'INFO':
									...
									switch($level) {
									case 2:
										list($cur, $total) = explode(' ', $rest);
										$status['html'] = t("Processed volume %s of %s", $cur, $total);
										break;
									case 7:
										preg_match("#'(?P<path>.*)' (?P<type>\w+)#", $rest, $m);
										$path = $m['path'];
										if( $m['type'] == 'reg' ) {
											$status['html'] = t("Restoring file %s", $path);
										} elseif( $m['type'] == 'dir' ) {
											$status['html'] = t("Creating directory %s", $path);
										}
									}
									break;
								}
							...
							if(preg_match("#^\.(?P<comment>.*)$#mS", $line, $m)) {
								$comment .= "\n$m[comment]";
							}
						}
						if(preg_match("#^(?P<type>INFO|NOTICE|ERROR) (?P<level>\d+) (?P<rest>.*?)#mS", $line, $m)) {
					...
					$status['html'] = "Unable to open logfile";
					$status['error'] = true;
				}
