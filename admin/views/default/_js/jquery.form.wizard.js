	...
	$.widget("ui.formwizard", {
		...
		_disableNavigation : function(){
			...
			if(!this.options.disableUIStyles){
				...
				this.backButton.addClass("ui-state-disabled");
			}
		},
		...
		_enableNavigation : function(){
			...
			if($.trim(this.currentStep) !== this.steps.eq(0).attr("id")){
				this.backButton.button("enable").show();
				if(!this.options.disableUIStyles){
					this.backButton.removeClass("ui-state-disabled");
				}
			}

			this.nextButton.button("enable").show();
			if(!this.options.disableUIStyles){
