...
if(jQuery) (function($){
		...
		$.extend($.fn, {
				fileTree: function(o) {
					...
					if( o.spinnerImage == undefined ) o.spinnerImage = '/admin/views/default/_img/spinner16.png' ;
					...
					$(this).each( function() {
								...
								if( o.dragAndDrop ) {
									...
									all.bind( "drag", function( event ){
											...
											if( 
												...
												event.offsetX > ($(that).offset().left + $(that).outerWidth()) ||
												event.offsetY <= 0 ||
												event.offsetY >= window.height
											) {
												$(that).stop();
												$.dropManage(); 
											} else if( 
							...
							if( o.fakeTopRoot ) {
								$(this).html('<div class="jqueryFileTreeFakeRoot">.</div><div class="jqueryFileTreeRealRoot"><ul class="jqueryFileTree start"><li class="wait">' + o.loadMessage + '<li></ul></div>');
