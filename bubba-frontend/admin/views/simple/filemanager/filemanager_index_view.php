...
<tr>
	<td width="3%">
		...
		<input type="checkbox" name="file_id[]" value="<?=$k++?>"/>
	</td>
	<td width="3%">[DIR]</td>
	<td width="57%">
		<a href="<?=FORMPREFIX?>/filemanager/cd<?=$path."/".($dir[3])?>"><? print substr($dir[3], 0, 50)?></a>
	</td>
	<td width="10%"align="right"><?=$dir[1]?></td>
...
<td>
	...
	<select id="sel_action" name="action" onChange="doChange()">
		...
		<option value="download"><?=t('Download as zip')?></option>
...
<td><input type="submit" name="mkdir" value="<?=t('Create folder')?>"/></td>
<td>
	<? if($ftd_running){?>
		<script type="text/javascript">
			var index=0;
		...
		<input type="button" value="Upload files" onclick="pop('<?=FORMPREFIX?>/upload/index<?=$path?>',500,150)" />
	<? } ?>
</td>
</tr>           
</table>
